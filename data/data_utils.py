...
def rescale(image, labels):
    ...
    # move
    trans_pair = [(x - cent_x, y - cent_y) if x != -1 and y!= -1 else (x, y) for x, y in keypt_pair]
    # rescale
    scaled_pts = [(x * rescale_factor, y * rescale_factor) if x != -1 and y != -1 else (x, y) for x, y in trans_pair]
    # move back
    final_points = [(x + cent_x, y + cent_y) if x != -1 and y != -1 else (x, y) for x, y in scaled_pts]
    ...
    # print("rescale points",return_keypt)
...
def shift(image, labels):
    ...
    # shift matrix
    trans_mat = np.float32([[1, 0, shift_y], [0, 1, shift_x]])
    image = cv2.warpAffine(image, trans_mat, (image.shape[1], image.shape[0]))
    # print(image.shape)
    # labels
    shifted_points = [(x + shift_y, y + shift_x) if x != -1 and y != -1 else
                      (x, y) for x, y in zip(labels[0::2], labels[1::2])]
    ...
    # print("shift points",return_keypt)
    return image, return_keypt
